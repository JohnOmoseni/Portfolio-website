...
const animateHeader = {
  hidden: { opacity: 0 },
  visible: { opacity: 1, transition: { type: "tween", ease: "easeOut", duration: 1, delay: 0.8 } },
};
...
function Header({ setOpenMenu, elem }) {
  const downloadResume = e => {
    ...
    fetch("/assets/myresume.pdf")
      ...
      .then(res => {
        ...
        link.setAttribute("download", "Johnny Omoseni Resume");
  ...
  return (
    <motion.header ref={elem} variants={animateHeader} initial="hidden" animate="visible">
      ...
      <button className="resume-btn" onClick={downloadResume} title="Download my resume">
        RESUME
      </button>
    </motion.header>
  );
